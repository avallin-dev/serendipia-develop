...
interface FileUploaderProps extends React.HTMLAttributes<HTMLDivElement> {
  ...
  /**
   * Whether the uploader has a advice.
   * @type boolean
   * @default true
   * @example advice
   */
  ...
  const onDrop = React.useCallback(
    (acceptedFiles: File[], rejectedFiles: FileRejection[]) => {
      ...
      if (onUpload && updatedFiles.length > 0 && updatedFiles.length <= maxFiles) {
        const target = updatedFiles.length > 0 ? `${updatedFiles.length} archivos` : `archivo`
          ...
          >
            ...
            {isDragActive ? (
              <div className="flex flex-col items-center justify-center gap-4 sm:px-5">
                ...
                <p className="font-medium text-muted-foreground">Suelta archivos aqui</p>
              </div>
            ) : (
              <div className="flex flex-col items-center justify-center gap-4 sm:px-5">
                <div className="rounded-full border border-dashed p-3">
