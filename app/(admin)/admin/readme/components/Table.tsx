...
import DeleteReadme from './DeleteReadme'
...
export default function TableMembership() {
  ...
  const [selectedReadme, setSelectedUser] = useState<ReadmeType>()
  const { readme, isLoading, isFetching } = useReadme()

  const [sorting, setSorting] = useState<SortingState>([])
  const [columnFilters, setColumnFilters] = useState<ColumnFiltersState>([])
  const [columnVisibility, setColumnVisibility] = useState<VisibilityState>({})
  const [rowSelection, setRowSelection] = useState({})
  const [filteredData, setFilteredData] = useState<ReadmeType[]>([])

  useEffect(() => {
    if (!isLoading && readme) {
      setFilteredData(readme)
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [isLoading, isFetching])

  const columns = useMemo<ColumnDef<ReadmeType, unknown>[]>(
    () => [
      {
        id: 'id',
        accessorKey: 'id',
      ...
      },
      {
        ...
        cell: ({ row }) => {
          ...
          if (file)
            return (
              <Link target="_blank" href={file} className="font-semibold text-blue-800 underline">
                Archivo
              </Link>
            )
        },
            ...
            <div className="flex justify-center gap-2">
              <Button
                ...
                onClick={() => {
                  ...
                  setSelectedUser(row.original as ReadmeType)
                }}
              >
      ...
      <div className="flex items-center justify-end gap-x-4 py-4">
        <div className="flex gap-4">
          <Dialog>
            ...
            {createIsOpen && (
              <CreateReadme open={createIsOpen} onClose={() => setCreateIsOpen(false)} />
            )}
            {updateIsOpen && (
              <UpdateReadme
                open={updateIsOpen}
